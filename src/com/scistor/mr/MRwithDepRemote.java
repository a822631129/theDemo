...
public class MRwithDepRemote extends Configured implements Tool {

	public static void main(String[] args) throws Exception {
		System.out.println("====这是一个REDUCE中依赖third.jar的MR====");
		int res = ToolRunner.run(new Configuration(), new MRwithDepRemote(),
				args);
		System.exit(res);
	...
	public int run(String[] arg) throws Exception {
		...
		// System.out.println("====-D参数aaa为===="
		// + this.getConf().getStrings("aaa")[0]);
		...
		final Job job = new Job(conf, MRwithDepRemote.class.getSimpleName());

		// 1.1指定读取的文件位于哪里
		FileInputFormat.setInputPaths(job, arg[0]);
		// 指定如何对输入文件进行格式化，把输入文件每一行解析成键值对
		// job.setInputFormatClass(TextInputFormat.class);
		job.setJarByClass(MRwithDepRemote.class);
		// 1.2 指定自定义的map类
		...
		// for (String entry : this.getConf().getStrings("aaa")) {
