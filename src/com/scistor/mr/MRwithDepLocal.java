...
public class MRwithDepLocal extends Configured implements Tool {

	public static void main(String[] args) throws Exception {
		System.out.println("====这是一个client中依赖third.jar的MR====");
		int res = ToolRunner.run(new Configuration(), new MRwithDepLocal(),
				args);
		System.exit(res);
	...
	public int run(String[] arg) throws Exception {
		...
		final Job job = new Job(conf, MRwithDepLocal.class.getSimpleName());

		// 1.1指定读取的文件位于哪里
		FileInputFormat.setInputPaths(job, arg[0]);
		// 指定如何对输入文件进行格式化，把输入文件每一行解析成键值对
		// job.setInputFormatClass(TextInputFormat.class);
		job.setJarByClass(MRwithDepLocal.class);
		// 1.2 指定自定义的map类
