...
public class ScistorApplicationMaster {

	private static final Log LOG = LogFactory
			.getLog(ScistorApplicationMaster.class);
	...
	private ArrayList<ScistorJar> scistorJars = new ArrayList<ScistorJar>();
	// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
	...
	private static final String ExecShellStringPath = ScistorClient.SCRIPT_PATH
			+ ".sh";
	private static final String ExecBatScripStringtPath = ScistorClient.SCRIPT_PATH
			+ ".bat";
	...
	public boolean init(String[] args) throws ParseException, IOException {
		...
		if (fileExist(log4jPath)) {
			try {
				Log4jPropertyHelper.updateLog4jConfiguration(
						ScistorApplicationMaster.class, log4jPath);
			} catch (Exception e) {
		...
		if (!fileExist(shellCommandPath)
				&& envs.get(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTLOCATION)
						.isEmpty()) {
			throw new IllegalArgumentException(
		...
		for (String key : envs.keySet()) {
			if (key.contains(ScistorDSConstants.DISTRIBUTEDJARLOCATION)) {
				ScistorJar scistorJar = new ScistorJar();
				scistorJar.setJarPath(envs.get(key));
				String num = key
						.split(ScistorDSConstants.DISTRIBUTEDJARLOCATION)[1];

				scistorJar
						.setTimestamp(Long.parseLong(envs
								.get(ScistorDSConstants.DISTRIBUTEDJARTIMESTAMP
										+ num)));
				scistorJar.setSize(Long.parseLong(envs
						.get(ScistorDSConstants.DISTRIBUTEDJARLEN + num)));

				scistorJars.add(scistorJar);
			}
		}
		// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
		if (envs.containsKey(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTLOCATION)) {
			scriptPath = envs
					.get(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTLOCATION);

			if (envs.containsKey(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTTIMESTAMP)) {
				shellScriptPathTimestamp = Long
						.valueOf(envs
								.get(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTTIMESTAMP));
			}
			if (envs.containsKey(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTLEN)) {
				shellScriptPathLen = Long.valueOf(envs
						.get(ScistorDSConstants.DISTRIBUTEDSHELLSCRIPTLEN));
			}
	...
	static class NMCallbackHandler implements NMClientAsync.CallbackHandler {
		...
		private final ScistorApplicationMaster applicationMaster;

		public NMCallbackHandler(ScistorApplicationMaster applicationMaster) {
		...
		public void onContainerStarted(ContainerId containerId,
			...
			try {
				ScistorApplicationMaster.publishContainerStartEvent(
	...
	private class LaunchContainerRunnable implements Runnable {
		...
		public void run() {
			...
			for (ScistorJar perJar : scistorJars) {
